...
for eid = 1:length(graph.edges)
    temp1 = getfield(graph.idLookup, num2str(graph.edges(eid).from));
  graph.edges(eid).fromIdx = temp1.offset + 1;
  temp2 = getfield(graph.idLookup, num2str(graph.edges(eid).to));
  graph.edges(eid).toIdx = temp2.offset + 1;
end
