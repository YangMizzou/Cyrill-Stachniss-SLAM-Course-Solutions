function plot_state(mu, sigma, landmarks, timestep, observedLandmarks, z, window)
    ...
    plot(cell2mat(L(2,:)), cell2mat(L(3,:)), 'k+', 'markersize', 6, 'linewidth', 0.5);
    for(i=1:length(observedLandmarks))
	if(observedLandmarks(i))
	    plot(mu(2*i+ 2),mu(2*i+ 3), 'bo', 'markersize', 6, 'linewidth', 0.5)
	...
	mX = mu(2*z(i).id+2);
	mY = mu(2*z(i).id+3);
    	line([mu(1), mX],[mu(2), mY], 'color', 'k', 'linewidth', 0.5);
    end
